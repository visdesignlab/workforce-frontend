    ...
    if( isinstance(s, dict) ):
        ...
        if( ((option =='ideal_staffing') | (option == 'ideal_staffing_current')) & \
           (sub_option == "all_combination")  ):
            ...
            p = tmp.groupby('svc_category')[provider_list['provider_abbr']].sum()
            current_demand = s['FTE'].iloc[:,getthis]

        if( (sub_option == "wage_weight" ) | (sub_option == "wage_max" ) | (option =='service_allocation') ):
            tmp = s['detail_f2f_mini']
            p = tmp.groupby('svc_category')[provider_list['provider_abbr']].sum()
            current_demand = s['FTE']

    s['detail_f2f_mini'] = tmp.to_dict()
    s['FTE'] = ''
    s['supply'] = current_supply.to_dict()
    s['demand'] = current_demand.to_dict()
    s['F2F'] = p.to_dict();
    return json.dumps(s)
...
def input_create(geo, year, sut_target, sdoh_score, pop_chronic_trend,  pop_chronic_prev, chron_care_freq,
    ...
    if( sut_target > 0):
        for col in provider_list['provider_abbr']:
            ser_prov[col] = ser_prov[col].replace(r'^\s*$', np.nan, regex=True).astype(float)
